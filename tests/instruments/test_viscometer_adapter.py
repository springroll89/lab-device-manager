from lab_device_manager.instruments.viscometer import (
    ViscometerAdapter,
    checksum_sum256,
    HEADER,
    MAX_BUFFER_BYTES,
    TRAILER,
)


def _frame(viscosity, temp, shear_rate, shear_stress, torque_pct):
    visc = int(round(viscosity * 100)).to_bytes(4, "big")
    t = int(round(temp * 10)).to_bytes(2, "big")
    sr = int(round(shear_rate * 1000)).to_bytes(4, "big")
    ss = int(round(shear_stress * 1000 * 10)).to_bytes(4, "big")
    tq = int(round(torque_pct * 10)).to_bytes(2, "big")
    partial = (HEADER + b"\x20" + visc + b"\x20" + t + b"\x20" + sr
               + b"\x20" + ss + b"\x20" + tq + b"\x20")
    return partial + bytes([checksum_sum256(partial)]) + TRAILER


class FakeTransport:
    def __init__(self, chunks):
        self.chunks = list(chunks)
        self.reset_count = 0

    def read_wait(self, timeout):
        if self.chunks:
            return self.chunks.pop(0)
        return b""

    def reset_connection(self):
        self.reset_count += 1


def test_identity():
    assert ViscometerAdapter(FakeTransport([])).identity() == "Fangrui Viscometer"


def test_read_status_returns_latest_frame():
    # two frames arrive in two reads; adapter drains buffer and returns the LATEST (f2)
    f1 = _frame(100, 20, 5, 10, 30)
    f2 = _frame(250.5, 25.5, 12.5, 33.3, 50.0)
    tr = FakeTransport([b"\x00" + f1 + b"\xaa", f2])
    s = ViscometerAdapter(tr, read_timeout=0.5).read_status()
    assert s.state == "running"
    assert s.device_id == "Fangrui Viscometer"
    assert s.temp_c == 25.5
    assert s.metrics["viscosity_mPas"] == 250.5
    assert s.metrics["shear_rate_1s"] == 12.5
    assert s.metrics["shear_stress_mPa"] == 33300.0
    assert s.metrics["shear_stress_Pa"] == 33.3
    assert s.metrics["torque_pct"] == 50.0
    assert s.metrics["data_verified"] is True
    assert s.metrics["checksum_validated"] is True


def test_read_status_offline_when_no_frame():
    tr = FakeTransport([b"\x00\x01\x02"])     # noise, no complete frame
    s = ViscometerAdapter(tr, read_timeout=0.05).read_status()
    assert s.state == "offline"


def test_read_status_offline_when_silent():
    s = ViscometerAdapter(FakeTransport([]), read_timeout=0.05).read_status()
    assert s.state == "offline"


def test_read_status_waits_for_first_frame_across_an_initial_stream_gap():
    frame = _frame(2.44, 0.0, 132.0, 0.322, 0.8)
    transport = FakeTransport([b"", frame])

    snapshot = ViscometerAdapter(transport, read_timeout=0.05).read_status()

    assert snapshot.state == "running"
    assert snapshot.metrics["viscosity_mPas"] == 2.44


def test_repeated_missing_frames_reset_a_stale_tcp_session():
    transport = FakeTransport([])
    adapter = ViscometerAdapter(
        transport,
        read_timeout=0.001,
        reconnect_after_misses=2,
    )

    assert adapter.read_status().state == "offline"
    assert transport.reset_count == 0

    assert adapter.read_status().state == "offline"
    assert transport.reset_count == 1


def test_noise_buffer_is_bounded_when_no_complete_frame():
    adapter = ViscometerAdapter(
        FakeTransport([b"\x01" * (MAX_BUFFER_BYTES + 100)]),
        read_timeout=0.05,
    )
    assert adapter.read_status().state == "offline"
    assert len(adapter._buffer) <= MAX_BUFFER_BYTES
