...
- **实物全程追溯**：为原材料容器、实验批次、中间品、成品和存储位置生成 URL 二维码标签，记录领用、库存扣减、暂存、取回、打印与补打事件。
...
- 标签二维码改为稳定的 `/scan/实物编号` URL，iPhone 系统相机可直接打开；`public_base_url` 用于避免把 `127.0.0.1` 印到标签中。
...
R-201 批次执行台入口：`/experiments`；原材料管理入口：`/materials`；
