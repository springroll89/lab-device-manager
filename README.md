...
化学品、耗材、办公用品共享 `PC-xxxx` 编号，扫码建档、入库、领用、盘点、隔离、处置与审计流水。
...
├── inventory/           # 统一库存服务、PubChem、旧库迁移与库存日报
...
当前测试覆盖：设备解码、Modbus 帧、Repository、运行时引擎、Web API、认证流程、R-201 状态机、幂等事件、温度检查点、粘度终点、偏差闭环、四设备固定 fixture，以及库存事务、权限、告警、导出、旧库迁移和实验用料联动。
