...
按采集主机、交换设备、串口网关和仪器展示实际接线关系，并实时显示端口、指标与分层通讯状态。
...
- ☀️ **浅色 / 深色双主题** — PC 与平板默认使用暖白浅色界面，也可从账号菜单切回深色模式；当前浏览器会记住选择。
...
| 🌀 搅拌器 | HMS-C × 5 | Modbus RTU | ✅ 9600 8N1 单机链路已验证 |
| 💧 粘度计 | 方瑞 LVDV-2T | 自定义二进制串口 | ✅ 9600 8N1、实机帧、缩放与校验已验证 |
...
- 设备看板和设备详情的运行记录不再显示内部数字 ID，统一显示设备别名与配置名称，例如「粘度计（viscometer-1）」。
- 运行状态增加中文展示，可区分正常完成、系统重启中断、通讯中断、报警终止等情况；数据库仍保留原始状态码用于筛选与追溯。
- 最近运行和运行详情按设备类型展示主要数据：粘度计显示粘度，注射泵显示累计液量，搅拌器与温湿度设备显示温度。
- 粘度计运行详情改为粘度随时间变化曲线，不再误用注射泵流速模板；设备运行 PDF、CSV 和 Excel 导出同步补充设备名称、类型、中文状态及相应指标。

### 2026-08-04 · LVDV-2T 真机通讯验证

- 通过 FTDI FT231X USB-RS232 直连 Mac，确认方瑞 LVDV-2T 在 `9600、8N1` 下连续主动上传，无需厂家收费上位机软件即可采集。
- 现场 120 秒共收到 32 个完整帧、927 字节；真机帧为 29 字节，帧头 `D6 C8 18 32`，帧尾 `13 AB`。
- 完成粘度计 → UT-6804 1 号口 → UT-6408 交换机 → Mac 的全链路验证；30 秒稳定收到 50 帧，校验错误为 0。
- UT-6804 现场参数固定为：1 号口 RS232 / 9600 8N1 / TCP `4001`，2 号口 RS485 / 9600 8E1 / TCP `4002`，3 号口 RS485 / 9600 8N1 / TCP `4003`。
- 修正旧协议文档导致的 28 字节 / `40` 帧尾解析错误，并加入真机原始帧回归测试。
- 通过同步显示屏对照，确认粘度÷100、温度÷10、剪切速率÷1000、剪切应力÷10、扭矩÷10；校验算法为前 26 字节累加后取低 8 位。
- 厂家 PDF 将剪切速率写为×100、扭矩写为×10000，与 LVDV-2T 真机不符；系统以同时抓帧与显示屏的实测结果为准。
- `0x32` 上传帧未包含转子编号和转速；这两项待后续确认其他帧类型，或作为测量条件记录。

### 2026-08-04 · PC 与平板浅色 / 深色双主题

- 全系统新增暖白浅色主题，覆盖登录、设备看板、实验执行、移动端执行模式、库存、危废、粘度工位、账号和设备详情；深色主题继续保留。
- 默认使用浅色模式，登录后可在账号菜单中切换；登录页和改密页提供独立切换按钮，当前浏览器会自动记住选择。
- 图表坐标、网格、提示框和状态颜色跟随主题更新，浅色模式使用更高对比度的文字、输入框和状态色。
- 新增浅底 Puricore 品牌图片；浅色模式自动使用浅底图，深色模式继续使用原黑底图。
- 移除设备看板右上角仅展示、不可操作且其他页面不存在的「本地实验室」标签，保持主导航一致。

### 2026-08-04 · 设备网络拓扑看板

- 设备看板新增「拓扑视图 / 设备列表」双视图，并记住当前浏览器的选择；原有指标卡片和详情入口继续保留。
- 拓扑按实际接入关系展示后台采集主机、UT-6408 网络汇聚、UT-6804 串口网关及下挂仪器；同一网关 IP 的设备自动归组，本机串口设备单独列出。
- 每台设备显示网关串口号、TCP 端口、主要实时读数和通讯状态；网关节点汇总下挂通道，可区分全部正常、部分通道异常和网关离线。
- `config.toml` 新增拓扑名称及 `gateway_name`、`gateway_model`、`gateway_port` 配置，现场增加设备或网关时无需修改页面代码。
- 桌面端采用横向网络结构，平板和手机自动切换为纵向布局，便于现场查看与点击。
...
- 方瑞粘度计的 9600 8N1、29 字节帧结构、五个数值字段、缩放和累加校验均已完成真机屏幕对照，合法帧可作为 `device_confirmed` 数据。
...
- **当前边界**：应用只读取设备数据，不向设备自动下发启停、转速、温度或泵速指令；粘度计当前的 `0x32` 帧不含转子编号与转速；当前默认一台设备占用一个串口。
