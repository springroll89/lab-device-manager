...
实验室实验执行、设备采集与物品库存平台 —— 统一接入注射泵、粘度计、搅拌器等串口/Modbus 仪器，并在同一账号、数据库和扫码入口下管理实验过程、原材料、耗材与办公用品。

> 本项目默认监听实验室局域网（`0.0.0.0:7800`），供平板和 PC 浏览器访问；请通过防火墙限制在受控内网，勿直接暴露到公网。
...
- **补录/Tag 修正**：可对已结束运行补充 project_tag、experiment_tag、remark；操作员由当前登录账号自动写入。
- **R-201 批次执行台**：以 Batch ID 串联 CEM/AEM 湿化学合成步骤、人工时间戳、设备数据源、物料、粘度、偏差与记录锁定。
- **实物全程追溯**：为原材料容器、实验批次、中间品、成品和存储位置生成离线文本二维码标签，记录领用、库存扣减、暂存、取回、打印与补打事件。
- **统一物品与库存**：化学品、耗材和办公用品共享 `PC-xxxx` 编号、扫码建档、入库、领用、盘点、隔离、处置、告警与审计流水；实验物料消耗直接写入同一库存账。
- **并行实验工作台**：同一操作员可切换多个进行中批次；搅拌器/注射泵按物理阶段排他占用，共享粘度计按样品短租约排队。
- **平板摄像头扫码**：优先使用浏览器本地识码；iPhone 等不支持浏览器识码 API 时，通过局域网服务端 ZXing 解码，图片不落盘。
- **断网幂等重试**：平板写操作先进入当前账号专属的本地 outbox，网络或服务器恢复后使用同一 `client_event_id` 自动重放。
...
- 原材料二维码和标签增加登记实验人；库存列表可一次生成当前筛选结果的全部瓶贴，库位也支持批量打印。首次盘点的 34 张瓶贴和 7 张库位贴均已通过逐张生成与解码验证。
...
tls_certfile       = "/path/to/lab-device.crt"
...
启动后服务器电脑会自动打开本机页面。平板连接同一局域网后，使用
`public_base_url` 配置的地址访问；不使用摄像头的临时测试也可以打开
`http://服务器局域网IP:7800`。
...
R-201 批次执行台入口：`/experiments`；物品与库存入口：`/inventory`
（旧 `/materials` 地址仍可使用）；
共享粘度计入口：`/measurement-station`。
...
当前测试覆盖设备解码、Modbus 帧、Repository、运行时引擎、Web API、认证流程、R-201 状态机、幂等事件、温度检查点、粘度终点、偏差闭环、四设备固定 fixture，以及库存事务、权限、告警、导出、旧库迁移和实验用料联动。
